  ...
  $bulan = $this->uri->segment(4);
          $tanggal  =$this->db->query("SELECT tanggal, SUM(volume) as total_volume FROM volume_tps LEFT JOIN master_tps ON volume_tps.kode_tps = master_tps.Kode_tps WHERE Wilayah = '$val[kota]' AND SUBSTR(tanggal,6,2) = $bulan AND SUBSTR(tanggal,1,4) = $tahun   AND (status = 2 OR status = 3) GROUP BY SUBSTR(tanggal,9,2), SUBSTR(tanggal,1,4)")->result_array();
